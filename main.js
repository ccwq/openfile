...
const { mergeMarkdownFiles } = require('./file-merge');
 dotenv.config();
...
console.log('正在运行下载器...');
try {
//   execSync(`node downloader.js`, { stdio: 'inherit' });
} catch (error) {
...
try {
//   execSync(`node html2md.js`, { stdio: 'inherit' });
} catch (error) {
...
try {
    ...
    const mdOutputDir = inputDir.endsWith('-markdown') ? inputDir : inputDir+"-markdown";
    ...
    try {
      ...
      mergeMarkdownFiles(mdOutputDir, 'merged.md');
    } catch (error) {
